...
\section{Nested Data Parallelism}
  ...
  In the ground breaking work \cite{Blelloch1996}
  major contributions to parallel programming in
  functional programming languages were made.
  The paper presented his earlier work \cite{NepaBlelloch1993} on NESL
  ...
  \footnote{Papers on NDP in Haskell: \cite{Harness2008}, \cite{DPHStatus2007},
  \cite{LeshchinskiyPhD2005}, \cite{HighOrdFlat2006}, \cite{DistTypes1999} and \cite{EffiVect2012Lipp}.}
  ...
  \paragraph{}
    ...
    \nomenclature{Flat Data Parallelism}{
    Expression of parallelism
    via parallel primitives over flat arrays.
    The primitives cannot be nested
    }
    \nomenclature{Nested Data Parallelism}{
    Expression of parallelism
    via parallel primitives over nested arrays.
    The primitives can be nested. It flattens the code
    before execution
    }
    ...
    \nomenclature{Chunk}{
    An local array (or linked list) that is part of a
    globally distributed array (or linked list)
    }
    
    \nomenclature{PU}{
    A processing unit. Refers mostly to CPUs on a
    single machine. Can also be CPUs in a cluster
    or in a distributed setting
    }
    ...
    \begin{enumerate}
      ...
      \item \emph{Stream Fusioning} -
        ...
        It uses \type{Vector a} and \c{Stream a}\footnotemark to
        implement the local array chunks and to fuse loops, respectively.
    \end{enumerate}
    
    \footnotetext{\c{Stream} is a data structure
    designed to specifically enable non-recursive
    definitions of fusing functions (e.g. \c{mapP}) by
    moving the recursion from the function
    to a step-wise stream-ful iteration. See \cite{GenVectorFusion2013}.}
    
    \nomenclature{Vectorization}{
    A compiler transformation which flattens and uses Fusion
    a nested data parallel program to increase
    cache locality, decrease communication and traversals
    for higher performance
    }
    
    \nomenclature{Fusion}{
    The use of Inlining and semantic-preserving Rewrite Rules
    to find and eliminate communication (Communication Fusion)
    or intermediate data structures (Stream Fusion)
    }
  ...
  \subsection{Vectorization}
    ...
    \subsubsection{Type dependent representation}
      ...
      be implemented equally. The sub-arrays could be of different size.
      ...
      and is directly linked to Cache Locality. \cite{Locality2005}.
  \footnotetext{e.g. \c{zipWithP (+) [:1,2,3:] [:1,3,4:] = [:1+1,2+3,3+4:]}. The arrays as assumed to be of equal size.}
      ...
      \footnote{\c{[\string#1,2,3\string#]} is the notation used for a contiguous-memory byte-array.}
      
      \nomenclature{Cache Locality}{
      Use of contiguous-memory data over
      pointer-based data to create an
      execution behaviour that simplifies
      CPUs cache hierarchies to optimise for
      and greatly increases performance
      }
...
array = AArr {
  ...
  lengths = [# 3,2,0,1 #]
      ...
      The functions \c{concatP} and \c{unconcatP}
...
mapPL f xss =
  ...
  $ xss
    ...
    in \cite{LeshchinskiyPhD2005} and \cite{HighOrdFlat2006}.
    ...
    \begin{figure}[h!]
        ...
        \end{center}
    \end{figure}
    \footnotetext{The commas in-between the elements have been omitted for visual clarity.
...
myFunc = joinD . mapD (mapS (mult2 . incr)) . splitD
  ...
  \subsection*{Functions and Rewrite Rules}
    ...
    \nomenclature{
    \c{Vector}}{is the Haskell implementation of conventional local in-memory arrays.
    They are used for the distributed chunks of \pad
    }
...
\section{Parallel Complexity Measures}
  ...
  \nomenclature{Work and Depth}{
  Language-based measures for the time
  necessary if given one processor (work) or infinitely many (depth)
  ...
  The work and depth of \c{mapP} embodies the idea,
  ...
  These measures, as introduced in \cite{Blelloch1996}, work
  ...
  the work and depth complexities will be
  presented mostly without explicit derivation.
  \comment{
...
\section{\ac: \algo}
  ...
  This section will introduce \algo \cite{ImageProcBook}. It will state the problem and give an overview of its solution.
  ...
  \subsection*{Problem}
    ...
    \begin{figure}[h]
      ...
      \caption[Unbalanced Image]{An image with low contrast}
      \label{fig:img-org}
    \end{figure}
    The goal is to make details more visible to the human viewer.
    Inspecting the images histogram reveals new insights.
    \footnote{made by Wikipedia user \c{Jarekt}. Source: \url{https://en.wikipedia.org/wiki/File:Unequalized_Histogram.svg}}
    
    \begin{figure}[h]
      \centering
      \includegraphics[width=0.45\textwidth]{hist-org}
      \caption[Unbalanced Histogram]{The histogram of the original image with absolute(red) and accumulative(black) count.}
      \label{fig:hist-org}
    \end{figure}
