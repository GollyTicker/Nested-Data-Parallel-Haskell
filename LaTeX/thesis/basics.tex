...
\section{Nested Data Parallelism}
  ...
  \paragraph{}
    ...
    to a step-wise stream-ful recursion. See \cite{GenVectorFusion2013}.}
  ...
  \footnotetext{e.g. \c{zipWithP (+) [:1,2,3:] [:1,3,4:] = [:1+1,2+3,3+4:]}. The arrays as assumed to be of equal size.}
      ...
      \footnote{\c{[\string#1,2,3\string#]} is the notation used for a contiguous-memory bytearray.}
...
mapPL f xss =
  ...
  $ xss
    ...
    The call to \c{incrL} happens for a flat array as shown in figure \ref{figure:fdpvsndp}.
    ...
    \begin{figure}[h!]
        ...
        \end{center}
    \end{figure}
    \footnotetext{Here, the commas in-between the elements have been omitted for visual clarity.
